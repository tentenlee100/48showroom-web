  ...
  return [
    {
      ...
      type: "lastIdol",
      name: "Last Idol ラストアイドル"
    }
  ]
}
function initSetting() {
  const setting = {
    userAllow: initAllAllowType().map(obj => {
      return {type: obj.type, show: true}
    }),
    showLowUrl: false,
    showImage: true
  }
...
function initUserAllow() {
  ...
  try {
    var needSaveAllow = false
    const allType = initAllAllowType()
    for (var index in allType) {
      const thisTypeFilter =  setting.userAllow.filter(obj => obj.type == allType[index].type)
      if (thisTypeFilter.length > 0 && thisTypeFilter[0].show == true ) {
        userAllow.push(allType[index].type)
      }else{
        userAllow.push(allType[index].type)
        needSaveAllow = true
      }
    ...
    if (needSaveAllow){
      saveSettingToStorge({
        userAllow: allType.map(obj => {
          return {
            type: obj.type,
            show: userAllow.indexOf(obj.type) > -1
              ? true
              : false
          }
        })
    ...
    // userAllow = setting.userAllow.filter(obj => {
  ...
  [types.ADD_USER_AllOW](state, data) {
    ...
    saveSettingToStorge({
      userAllow: initAllAllowType().map(obj => {
        return {
          type: obj.type,
          show: state.userAllow.indexOf(obj.type) > -1
            ? true
            : false
        }
      })
